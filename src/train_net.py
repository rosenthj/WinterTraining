...
def parse_args():
    ...
    parser.add_argument('--optimizer', type=str, default="sgd", choices=["sgd"],
                        help="Optimizer: sgd (SGD with --momentum)")
    ...
    parser.add_argument('--persistent-optimizer', action='store_true', default=False,
                        help="Keep one optimizer across the whole LR schedule (just change its LR "
                             "per step) instead of recreating it each step, carrying the momentum "
                             "buffers across LR drops.")
    parser.add_argument('--reg-weights-only', action='store_true', default=False,
                        help="Apply weight decay only to Linear/Conv weights, exempting biases "
                             "and bias-like params (e.g. b1). Only matters with --weight-decay.")
